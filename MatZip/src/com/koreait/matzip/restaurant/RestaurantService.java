...
public class RestaurantService {
	...
	public int addRecMenus(HttpServletRequest request) {
		...
		// request.getServletContext()를 하면 application으로 넘어옴
		// 지금 현재 돌아가고 있는 걔의 주소값??
		// 현재 서버가 돌아가고 있는 위치의 절대 주소값이 넘어감(소스파일과 다름)
		String tempPath = savePath + "/temp";	// 임시	
		FileUtils.makeFolder(tempPath); // 폴더가 없으면 만들고 있으면 안 만듦
		...
		try {
			...
			String originFileNm = "";
			// Enumeration - list와 비슷, 인터페이스
			Enumeration files = multi.getFileNames();
			while(files.hasMoreElements()) { // rs.next()의 boolean 기능만 함
				String key = (String)files.nextElement(); // rs.next()의 다음 파일 가리키기 기능만 함
				System.out.println("key : " + key); // input의 name
				originFileNm = multi.getFilesystemName(key); // 업로드한 파일의 original 파일명
